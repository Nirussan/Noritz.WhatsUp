...
import com.noritz.whatsup.databinding.DeleteMessageBinding
import com.noritz.whatsup.databinding.SendMessageBinding
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder.javaClass == SentMsgHolder::class.java){
            ...
            if (message.message.equals("photo")) {
                ...
                Glide.with(context).load(message.imageUrl).placeholder(R.drawable.image).into(viewHolder.binding.image)
            }
            ...
            viewHolder.itemView.setOnLongClickListener {
                ...
                binding.deleteEveryone.setOnClickListener{
                    ...
                    message.messageId?.let { it->
                        FirebaseDatabase.getInstance().reference.child("chats").child(senderRoom).child("message").child(it).setValue(message)
                    }
                    message.messageId?.let { it->
                        FirebaseDatabase.getInstance().reference.child("chats").child(receiverRoom).child("message").child(it).setValue(message)
                    }
                    dialog.dismiss()
                }
                binding.deleteSolo.setOnClickListener{
                    message.messageId?.let { it->
                        FirebaseDatabase.getInstance().reference.child("chats").child(senderRoom).child("message").child(it).setValue(null)
                    }
                    dialog.dismiss()
                }
            ...
            viewHolder.itemView.setOnLongClickListener {
                ...
                binding.deleteEveryone.setOnClickListener {
                    ...
                    message.messageId?.let { it ->
                        FirebaseDatabase.getInstance().reference.child("chats").child(senderRoom)
                            .child("message").child(it).setValue(message)
                    }
                    message.messageId?.let { it ->
                ...
                binding.deleteSolo.setOnClickListener {
                    message.messageId?.let { it ->
                        FirebaseDatabase.getInstance().reference.child("chats").child(senderRoom)
                            .child("message").child(it).setValue(null)
                    }
                    dialog.dismiss()
                }
                binding.cancel.setOnClickListener { dialog.dismiss() }
